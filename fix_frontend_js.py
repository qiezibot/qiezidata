...
# Find the clouddata frontend section - from loadCloudDataProjects to the last loadDashboard()
start = c.find('function loadCloudDataProjects')
end = c.find('loadDashboard()')
end = c.find(';', end) + 1  # include semicolon
...
assert start >= 0, 'loadCloudDataProjects not found!'
assert end > start, f'end({end}) < start({start})'

c = c[:start] + new_js + c[end:]
print('Frontend JS fully replaced')
