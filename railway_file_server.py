...
document.getElementById('cdDataBody').addEventListener('click',function(e){var btn=e.target.closest('.cd-del-btn');if(!btn)return;var cid=parseInt(btn.getAttribute('data-cid'));if(confirm('确定要删除这条数据吗?'))deleteCD(cid);})
...
function downloadCD(cid){window.location.href='/admin/cddata/download/'+cid}
