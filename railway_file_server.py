...
ync function delCloudData(id){if(!confirm('确定删除?'))return;try{var r=await fetch('/admin/clouddata/'+id,{method:'DELETE',credentials:'include'});if(r.ok){loadCloudData();showToast('删除成功','success')}else if(r.status===401)window.location.href='/'}catch(e){}}
