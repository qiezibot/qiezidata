...
async def init_db():
    ...
    if use_pg:
        ...
        if 'user_id' not in cols:
            ...
            if not admin:
                h = hash_password('admin123')
                conn.execute("INSERT INTO users (username,password_hash,display_name,created_at,role) VALUES (?,?,?,?,?)",
                             ('admin', h, '管理员', datetime.utcnow().isoformat(), 'admin'))
...
async def get_user(uid):
    row = await db_fetchrow(
        'SELECT id,username,display_name,role FROM users WHERE id=$1' if use_pg else 'SELECT id,username,display_name,role FROM users WHERE id=?',
        uid
    )
...
async def home(request: Request):
    ...
    if user.get('role') == 'admin':
        return HTMLResponse(admin_html(user))
    return HTMLResponse(user_html(user))
...
async def upload_file(request: Request, file: UploadFile = File(...)):
    ...
    if use_pg:
        row = await db_fetchrow(
            'INSERT INTO files (user_id,filename,original_name,size,mime_type,file_path) VALUES ($1,$2,$3,$4,$5,$6) RETURNING id,upload_time',
...
# ---- 用户文件路由 ---- 
async def _list_user_files(uid):
    if use_pg:
        return await db_fetch(
            'SELECT id,filename,original_name,size,mime_type,upload_time FROM files WHERE user_id=$1 ORDER BY upload_time DESC', uid)
    return await db_fetch(
        'SELECT id,filename,original_name,size,mime_type,upload_time FROM files WHERE user_id=? ORDER BY upload_time DESC', uid)

@app.get('/files')
async def list_files(request: Request):
    uid = require_auth(request)
    rows = await _list_user_files(uid)
    return [{**dict(r), 'upload_time': r['upload_time'] if not use_pg else r['upload_time'].isoformat()} for r in rows]

async def _get_file(uid, fid):
    if use_pg:
        return await db_fetchrow('SELECT * FROM files WHERE id=$1 AND user_id=$2', fid, uid)
    return await db_fetchrow('SELECT * FROM files WHERE id=? AND user_id=?', fid, uid)

@app.get('/download/{fid}')
async def download_file(request: Request, fid: int):
    uid = require_auth(request)
    row = await _get_file(uid, fid)
...
async def delete_file(request: Request, fid: int):
    uid = require_auth(request)
    if use_pg:
        row = await db_fetchrow('SELECT * FROM files WHERE id=$1 AND user_id=$2', fid, uid)
    else:
        row = await db_fetchrow('SELECT * FROM files WHERE id=? AND user_id=?', fid, uid)
    if not row: raise HTTPException(status_code=404)
    if os.path.exists(row['file_path']): os.remove(row['file_path'])
    if use_pg:
        await db_execute('DELETE FROM files WHERE id=$1 AND user_id=$2', fid, uid)
    else:
        await db_execute('DELETE FROM files WHERE id=? AND user_id=?', fid, uid)
    return {'message': '删除成功'}
...
async def admin_files(request: Request):
    ...
    if use_pg:
        rows = await db_fetch('''
            SELECT f.id,f.original_name,f.size,f.upload_time,u.username as owner
            FROM files f LEFT JOIN users u ON f.user_id=u.id ORDER BY f.upload_time DESC
        ''')
    else:
        rows = await db_fetch('''
            SELECT f.id,f.original_name,f.size,f.upload_time,u.username as owner
...
# ===== 页面模板 =====
_LOGIN_HTML = '''<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>茄子数据</title>
...
.tabs{display:flex;margin-bottom:28px;border-bottom:2px solid #eee;gap:0}
.tab{flex:1;text-align:center;padding:12px;cursor:pointer;color:#999;font-weight:500;transition:.2s;border-bottom:2px solid transparent;margin-bottom:-2px}
.tab.active{color:#667eea;border-bottom-color:#667eea}.tab:hover{color:#667eea}
.form{display:none}.form.active{display:block}
.input-group{margin-bottom:18px;text-align:left}.input-group label{display:block;font-size:13px;color:#555;margin-bottom:5px;font-weight:500}
...
<div class="card"><h1>🍆 茄子数据</h1><p class="subtitle">文件管理服务 v2.1</p>
...
function validateRegister(){const p1=document.getElementById('regPass').value,p2=document.getElementById('regPass2').value,err=document.getElementById('regError');if(p1!==p2){err.textContent='两次密码不一致';err.className='msg error';err.style.display='block';return false}return true}
</script></body></html>'''

def admin_html(user):
    name = user.get('display_name', '') or user.get('username', '')
    return f'''<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>茄子数据 - 管理后台</title>
...
.header .user-area{{display:flex;align-items:center;gap:16px;font-size:14px}}
.header .user-area a{{color:#999;text-decoration:none;cursor:pointer}}
.header .user-area a:hover{{color:#e74c3c}}
...
.stat-card .num{{font-size:32px;font-weight:700;color:#667eea;margin-bottom:4px}}
...
.upload-zone .icon{{font-size:48px;margin-bottom:10px}}.upload-zone .hint{{font-size:12px;color:#aaa}}
.file-list{{list-style:none}}.file-item{{display:flex;justify-content:space-between;align-items:center;padding:12px 0;border-bottom:1px solid #f0f0f0}}
.file-item:last-child{{border-bottom:none}}.file-info{{flex:1}}.file-name{{font-weight:500;color:#333;word-break:break-all}}
...
.toast{{position:fixed;top:20px;right:20px;padding:12px 20px;border-radius:8px;color:#fff;font-size:14px;opacity:0;transition:.3s;z-index:999}}
.toast.show{{opacity:1}}.toast.success{{background:#27ae60}}.toast.error{{background:#e74c3c}}
...
@media(max-width:768px){{.sidebar{{width:160px;font-size:13px}}}}
</style></head><body>
<div class="header"><a href="/" class="logo">🍆 茄子数据</a><div class="user-area"><span>👤 {name}</span><span class="badge badge-admin">管理员</span><a href="/logout">退出</a></div></div>
<div class="layout"><div class="sidebar">
<a class="nav-item active" onclick="switchPage('dashboard',this)">📊 概览</a>
<a class="nav-item" onclick="switchPage('files',this)">📁 文件管理</a>
<a class="nav-item" onclick="switchPage('upload',this)">📤 上传文件</a>
<a class="nav-item" onclick="switchPage('users',this)">👥 用户管理</a>
<a class="nav-item" onclick="switchPage('about',this)">ℹ️ 关于</a>
</div><div class="main">
<div class="tab-page active" id="page-dashboard">
<div class="stats"><div class="stat-card"><div class="num" id="statUsers">-</div><div class="label">用户总数</div></div>
<div class="stat-card"><div class="num" id="statFiles">-</div><div class="label">文件总数</div></div>
<div class="stat-card"><div class="num" id="statSize">-</div><div class="label">总容量</div></div>
<div class="stat-card"><div class="num" id="statAdmin">-</div><div class="label">管理员</div></div></div>
</div>
<div class="tab-page" id="page-files">
<div class="card"><h2>📋 所有用户文件</h2><div id="fileList"><div class="empty-state"><div class="icon">📭</div><p>暂无文件</p></div></div></div>
</div>
<div class="tab-page" id="page-upload">
<div class="card"><h2>📤 上传文件</h2>
<div class="upload-zone" id="dropZone"><div class="icon">📁</div><p>拖拽文件到此处，或点击选择文件</p><p class="hint">支持任意格式，单文件最大 100MB</p><input type="file" id="fileInput" style="display:none"></div>
<progress id="uploadProgress" max="100" value="0"></progress>
<div id="uploadStatus" style="display:none"></div>
<div style="margin-top:16px"><h3 style="font-size:15px;margin-bottom:10px">我的文件</h3><div id="myFileList"><div class="empty-state"><p>暂无文件</p></div></div></div>
</div></div>
<div class="tab-page" id="page-users">
<div class="card"><h2>👥 用户管理</h2><table class="user-table"><thead><tr><th>ID</th><th>用户名</th><th>显示名</th><th>角色</th><th>注册时间</th></tr></thead><tbody id="userTableBody"></tbody></table></div>
</div>
<div class="tab-page" id="page-about">
<div class="card"><h2>ℹ️ 关于</h2><p style="line-height:1.8;color:#666;font-size:14px">🍆 茄子数据 v2.1<br>基于 FastAPI + PostgreSQL 构建<br>多用户文件管理系统<br><br>📌 默认管理员: admin / admin123<br>🔒 每个用户文件隔离</p></div>
</div>
</div></div>
<div id="toast" class="toast"></div>
<script>
function switchPage(id,el){{document.querySelectorAll('.nav-item').forEach(n=>n.classList.remove('active'));el.classList.add('active');document.querySelectorAll('.tab-page').forEach(p=>p.classList.remove('active'));document.getElementById('page-'+id).classList.add('active');if(id==='dashboard')loadDashboard();if(id==='files')loadAllFiles();if(id==='users')loadUsers();if(id==='upload')loadMyFiles()}}
const API='';
...
document.getElementById('dropZone')?.addEventListener('drop',e=>{{e.preventDefault();document.getElementById('dropZone').classList.remove('dragover');if(e.dataTransfer.files.length)uploadFile(e.dataTransfer.files[0])}});
document.getElementById('fileInput')?.addEventListener('change',()=>{{if(document.getElementById('fileInput').files.length)uploadFile(document.getElementById('fileInput').files[0])}});
async function uploadFile(file){const fd=new FormData();fd.append('file',file);const p=document.getElementById('uploadProgress'),s=document.getElementById('uploadStatus');p.style.display='block';s.style.display='block';s.textContent='上传中: '+file.name;const xhr=new XMLHttpRequest();xhr.upload.onprogress=e=>{if(e.lengthComputable)p.value=(e.loaded/e.total)*100};try{const r=await new Promise((resolve,reject)=>{xhr.onload=()=>{if(xhr.status===200)resolve(JSON.parse(xhr.responseText));else if(xhr.status===401)window.location.href='/';else reject(new Error(xhr.statusText))};xhr.onerror=()=>reject(new Error('网络错误'));xhr.open('POST',API+'/upload');xhr.withCredentials=true;xhr.send(fd)});showToast('上传成功: '+r.filename,'success');loadMyFiles()}catch(e){showToast('上传失败','error')}
p.style.display='none';s.style.display='none';document.getElementById('fileInput').value=''}
async function loadDashboard(){try{const r=await fetch(API+'/admin/stats',{credentials:'include'});if(r.status===401){window.location.href='/';return}const d=await r.json();document.getElementById('statUsers').textContent=d.users;document.getElementById('statFiles').textContent=d.files;document.getElementById('statSize').textContent=d.size_display;document.getElementById('statAdmin').textContent=d.admins}catch(e){}}
async function loadAllFiles(){try{const r=await fetch(API+'/admin/files',{credentials:'include'});if(r.status===401){window.location.href='/';return}const files=await r.json();const el=document.getElementById('fileList');if(!files.length){el.innerHTML='<div class=\"empty-state\"><div class=\"icon\">📭</div><p>暂无文件</p></div>';return}
el.innerHTML='<ul class=\"file-list\">'+files.map(f=>'<li class=\"file-item\"><div class=\"file-info\"><div class=\"file-name\">'+esc(f.original_name)+'</div><div class=\"file-meta\">'+fmtSize(f.size)+' 用户: '+esc(f.owner)+' '+f.upload_time+'</div></div><div class=\"file-actions\"><a href=\"/download/'+f.id+'\" download>⬇️</a><button class=\"del-btn\" onclick=\"delAdminFile('+f.id+')\">🗑️</button></div></li>').join('')+'</ul>'}catch(e){}}
async function delAdminFile(id){if(!confirm('确定删除？'))return;try{const r=await fetch(API+'/delete/'+id,{method:'DELETE',credentials:'include'});if(r.ok){showToast('删除成功','success');loadAllFiles()}else if(r.status===401)window.location.href='/'}catch(e){}}
async function loadUsers(){try{const r=await fetch(API+'/admin/users',{credentials:'include'});if(r.status===401){window.location.href='/';return}const users=await r.json();document.getElementById('userTableBody').innerHTML=users.map(u=>'<tr><td>'+u.id+'</td><td>'+esc(u.username)+'</td><td>'+esc(u.display_name||'-')+'</td><td><span class=\"badge '+(u.role==='admin'?'badge-admin':'badge-user')+'\">'+u.role+'</span></td><td>'+u.created_at+'</td></tr>').join('')}catch(e){}}
async function loadMyFiles(){try{const r=await fetch(API+'/files',{credentials:'include'});if(r.status===401){window.location.href='/';return}const files=await r.json();const el=document.getElementById('myFileList');if(!files.length){el.innerHTML='<div class=\"empty-state\"><p>暂无文件</p></div>';return}
el.innerHTML='<ul class=\"file-list\">'+files.map(f=>{const te=['.txt','.json','.xml','.py','.js','.sh','.yaml','.toml','.sql','.html','.css','.md','.csv','.log','.bat','.ps1'];const ext='.'+(f.original_name||'').split('.').pop();const cr=f.mime_type&&f.mime_type.startsWith('text/')||te.includes(ext);return '<li class=\"file-item\"><div class=\"file-info\"><div class=\"file-name\">'+esc(f.original_name)+'</div><div class=\"file-meta\">'+fmtSize(f.size)+' '+f.upload_time+'</div></div><div class=\"file-actions\">'+(cr?'<a href=\"/read/'+f.id+'\" target=\"_blank\">📖</a>':'')+'<a href=\"/download/'+f.id+'\" download>⬇️</a><button class=\"del-btn\" onclick=\"delMyFile('+f.id+')\">🗑️</button></div></li>'}).join('')+'</ul>'}catch(e){}}
async function delMyFile(id){if(!confirm('确定删除？'))return;try{const r=await fetch(API+'/delete/'+id,{method:'DELETE',credentials:'include'});if(r.ok){showToast('删除成功','success');loadMyFiles()}else if(r.status===401)window.location.href='/'}catch(e){}}
...
def user_html(user):
    name = user.get('display_name', '') or user.get('username', '')
    return f'''<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>茄子数据 - 我的文件</title>
<style>*{{margin:0;padding:0;box-sizing:border-box}}body{{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;background:#f0f2f5;min-height:100vh;color:#333}}
...
.header .logo{{font-size:20px;font-weight:700;color:#667eea;text-decoration:none}}
...
.upload-zone:hover,.upload-zone.dragover{{border-color:#667eea;background:#f8f9ff}}
.upload-zone .icon{{font-size:48px}}.upload-zone .hint{{font-size:12px;color:#aaa}}
.file-list{{list-style:none}}.file-item{{display:flex;justify-content:space-between;padding:12px 0;border-bottom:1px solid #f0f0f0}}
.file-item:last-child{{border-bottom:none}}.file-info{{flex:1}}.file-name{{font-weight:500}}.file-meta{{font-size:12px;color:#999}}
...
progress{{width:100%;height:6px;border-radius:3px;margin-top:10px;display:none}}
progress::-webkit-progress-bar{{background:#eee}}progress::-webkit-progress-value{{background:#667eea;border-radius:3px}}
.toast{{position:fixed;top:20px;right:20px;padding:12px 20px;border-radius:8px;color:#fff;font-size:14px;opacity:0;transition:.3s;z-index:999}}
.toast.show{{opacity:1}}.toast.success{{background:#27ae60}}.toast.error{{background:#e74c3c}}
</style></head><body>
<div class="header"><div><a href="/" class="logo">🍆 茄子数据</a></div><div class="user-area"><span>👤 {name}</span><a href="/logout">退出</a></div></div>
<div class="container">
<div class="card"><h2>📤 上传文件</h2>
<div class="upload-zone" id="dropZone"><div class="icon">📁</div><p>拖拽文件或点击选择</p><p class="hint">支持任意格式</p><input type="file" id="fileInput" style="display:none"></div>
<progress id="uploadProgress" max="100" value="0"></progress><div id="uploadStatus" style="display:none"></div></div>
...
document.getElementById('dropZone').addEventListener('drop',e=>{e.preventDefault();document.getElementById('dropZone').classList.remove('dragover');if(e.dataTransfer.files.length)uploadFile(e.dataTransfer.files[0])});
document.getElementById('fileInput').addEventListener('change',()=>{if(document.getElementById('fileInput').files.length)uploadFile(document.getElementById('fileInput').files[0])});
async function uploadFile(file){const fd=new FormData();fd.append('file',file);const p=document.getElementById('uploadProgress'),s=document.getElementById('uploadStatus');p.style.display='block';s.style.display='block';s.textContent='上传中: '+file.name;const xhr=new XMLHttpRequest();xhr.upload.onprogress=e=>{if(e.lengthComputable)p.value=(e.loaded/e.total)*100};try{const r=await new Promise((resolve,reject)=>{xhr.onload=()=>{if(xhr.status===200)resolve(JSON.parse(xhr.responseText));else if(xhr.status===401)window.location.href='/';else reject(new Error(xhr.statusText))};xhr.onerror=()=>reject(new Error('网络错误'));xhr.open('POST',API+'/upload');xhr.withCredentials=true;xhr.send(fd)});showToast('上传成功: '+r.filename,'success');loadFiles()}catch(e){showToast('上传失败','error')}
p.style.display='none';s.style.display='none';document.getElementById('fileInput').value=''}
async function loadFiles(){try{const r=await fetch(API+'/files',{credentials:'include'});if(r.status===401){window.location.href='/';return}const files=await r.json();const el=document.getElementById('fileList');if(!files.length){el.innerHTML='<div class=\"empty-state\"><p>暂无文件</p></div>';return}
el.innerHTML='<ul class=\"file-list\">'+files.map(f=>{const te=['.txt','.json','.xml','.py','.js','.sh','.yaml','.toml','.sql','.html','.css','.md','.csv','.log','.bat','.ps1'];const ext='.'+(f.original_name||'').split('.').pop();const cr=f.mime_type&&f.mime_type.startsWith('text/')||te.includes(ext);return '<li class=\"file-item\"><div class=\"file-info\"><div class=\"file-name\">'+esc(f.original_name)+'</div><div class=\"file-meta\">'+fmtSize(f.size)+' '+f.upload_time+'</div></div><div class=\"file-actions\">'+(cr?'<a href=\"/read/'+f.id+'\" target=\"_blank\">📖</a>':'')+'<a href=\"/download/'+f.id+'\" download>⬇️</a><button class=\"del-btn\" onclick=\"delFile('+f.id+')\">🗑️</button></div></li>'}).join('')+'</ul>'}catch(e){}}
async function delFile(id){if(!confirm('确定删除？'))return;try{const r=await fetch(API+'/delete/'+id,{method:'DELETE',credentials:'include'});if(r.ok){showToast('删除成功','success');loadFiles()}else if(r.status===401)window.location.href='/'}catch(e){}}
function fmtSize(b){if(b<1024)return b+'B';if(b<1024*1024)return (b/1024).toFixed(1)+'KB';return (b/(1024*1024)).toFixed(1)+'MB'}
function esc(s){return (s||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')}
function showToast(m,t){const el=document.getElementById('toast');el.textContent=m;el.className='toast '+t+' show';setTimeout(()=>el.classList.remove('show'),3000)}
loadFiles();
