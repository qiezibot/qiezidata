...
async function markCD(id,readVal){var t=readVal==true?'已读':'未读';if(!confirm('确定标记为'+t+'?'))return;try{var r=await fetch('/clouddata/mark/id/'+id+'?read='+readVal,{method:'POST',credentials:'include'});if(r.ok){loadCloudData();showToast('更新成功','success')}else if(r.status===401)window.location.href='/'}catch(e){}}
...
function showToast(m,t){var el=document.getElementById('toast');el.textContent=m;el.className='toast '+t+' show';setTimeout(function(){el.classList.remove('show')},3000)}async function addCloudData(){var k=document.getElementById("cdKey").value.trim();var v=document.getElementById("cdVal").value.trim();if(!k||!v)return;if(!confirm("确定添加 Key="+k+"?"))return;try{var r=await fetch("/admin/clouddata/add",{method:"POST",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify({key:k,value:v})});if(r.ok){document.getElementById("cdKey").value="";document.getElementById("cdVal").value="";loadCloudData();showToast("添加成功","success")}else if(r.status===401)window.location.href="/"}catch(e){}}
