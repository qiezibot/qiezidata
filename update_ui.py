...
print(f"ADMIN: {len(ADMIN)}, LOGIN: {len(LOGIN)}, USER: {len(USER)}")

# Verify
assert '_LOGIN = """\\' in new_content
assert '_ADMIN = """\\' in new_content
assert '_USER = """\\' in new_content
...
# Check keywords
for t in ['茄子', '登录', '仪表盘', '文件管理', '用户管理', '退出登录', '暂无文件']:
    assert t in new_content, f"Missing: {t}"
print("All Chinese text present!")
