...
print(f'ADMIN content: {adm_content_start} to {adm_end}, length: {adm_end - adm_content_start}')

adm = content[adm_content_start:adm_end]

# Check if pwdModal div exists
has_pwd_div = 'id="pwdModal"' in adm
print(f'Has id="pwdModal": {has_pwd_div}')

# Insert profileModal at the end of admin template (before </body></html>)
body_end = adm.find('</body></html>')
print(f'</body></html> at position {body_end} in template')
...
adm = adm[:body_end] + profile_html + adm[body_end:]
print(f'profileModal HTML inserted at position {body_end} in template')
...
has_pwd_div2 = 'id="pwdModal"' in t
print(f'Has id="profileModal": {has_prof_div}')
print(f'Has id="pwdModal": {has_pwd_div2}')
...
# Push
payload = {
    'message': 'Add profileModal HTML to admin template',
