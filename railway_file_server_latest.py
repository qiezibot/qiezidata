...
async def clouddata_add(request: Request):
    uid = _require(request); user = await _user(uid)
    if not user or user.get('role') != 'admin': raise HTTPException(status_code=403)
    b = await request.json(); k = b.get('key','').strip(); v = b.get('value','').strip(); pid = b.get('project_id')
    if not k or not v: raise HTTPException(400)
    if pid is not None:
        import hashlib
...
local url = "https://qiezidata-production.up.railway.app/api/cddata/2?token=xxx&amp;queryType=0"<br>
...
local resp = http.post(url, headers, body)<br>traceprint(resp.body)<br><br>
<strong>lua</strong> 获取单条数据（GET）<br>
local url = "https://qiezidata-production.up.railway.app/api/cddata/{data_id}?token=xxx"<br>
local resp = http.get(url, {})<br>traceprint(resp.body)<br><br>
...
local resp = http.get(url, {})<br>traceprint(resp.body)<br>
</div>
</div>
</div>
