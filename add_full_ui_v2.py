...
# 压缩：把 _ADMIN 模板内的所有连续空行合并为1个空行
html = re.sub(r'\n[ \t]*\n[ \t]*\n', '\n\n', html)

c = c[:start] + html + c[end:]

# 也压缩文件其他部分的空行（Python 代码间的空行保留一些，但过多也去掉）
# 先替换掉 Python 代码区域的连续空行（超过2个空行的都缩成1个）
# _ADMIN 之外的部分
before_admin = c[:start]
after_admin = c[end:]

# 压缩空行：超过2个连续空行的缩成1个
before_admin = re.sub(r'\n\n\n+', '\n\n', before_admin)
html = re.sub(r'\n\n\n+', '\n\n', html)
after_admin = re.sub(r'\n\n\n+', '\n\n', after_admin)

c = before_admin + html + after_admin
...
print(f'Lines: {len(lines)}, empty: {empty}')
