import re

with open('railway_file_server.py', encoding='utf-8') as f:
    c = f.read()

# 找 _ADMIN
m = re.search(r'_ADMIN\s*=\s*"""', c)
start = m.end()
endm = re.search(r'\n"""', c[start:])
end = start + endm.start()

html = c[start:end]

# ===== 从 _ADMIN 中提取所有 JS 到一个单独文件 =====
# 把所有 <script>...</script> 内容提取出来
def extract_scripts(html_str):
    script_blocks = []
    def replacer(m):
        tag = m.group(1)  # 可能是 <script>...</script> 或 <script src=...>...</script>
        content = m.group(2)
        # 保留 src 属性
        src_attr = ''
        sm = re.search(r'src=\S+', tag, re.IGNORECASE)
        if sm:
            src_attr = ' ' + sm.group()
        script_blocks.append(content)
        return f'<script{src_attr} src="/api/app.js"></script>'
    
    new_html = re.sub(r'<script([^>]*)>(.*?)</script>', replacer, html_str, flags=re.DOTALL)
    return new_html, script_blocks

new_html, scripts = extract_scripts(html)

# 所有 JS 合并
all_js = '\n'.join(scripts)

# 加上改密码的 JS 函数
all_js += '''
function openProfile(){var e=document.getElementById("profileModal");if(!e){var m=document.createElement("div");m.innerHTML='<div id="profileModal" class="modal" style="display:none"><div class="modal-content" style="max-width:420px"><span class="modal-close" onclick="closeProfile()">&times;</span><h3>\u4e2a\u4eba\u8d44\u6599</h3><div class="form-group"><label>\u7528\u6237\u540d</label><input id="profModalUser" type="text" readonly="" style="background:#f5f5f5;cursor:not-allowed"></div><div class="form-group"><label>\u663e\u793a\u540d\u79f0</label><input id="profModalDN" type="text" placeholder="\u8f93\u5165\u663e\u793a\u540d\u79f0"></div><div class="form-group"><label>\u89d2\u8272</label><input id="profModalRole" type="text" readonly="" style="background:#f5f5f5;cursor:not-allowed"></div><button class="btn" onclick="saveProfileModal()">\u4fdd\u5b58</button><div id="profModalSaveMsg" class="msg" style="display:none"></div><hr style="margin:16px 0"><h4>\u4fee\u6539\u5bc6\u7801</h4><div class="form-group"><label>\u65e7\u5bc6\u7801</label><input id="profModalOldPwd" type="password" placeholder="\u8f93\u5165\u65e7\u5bc6\u7801"></div><div class="form-group"><label>\u65b0\u5bc6\u7801</label><input id="profModalNewPwd" type="password" placeholder="\u81f3\u5c114\u4e2a\u5b57\u7b26"></div><div class="form-group"><label>\u786e\u8ba4\u65b0\u5bc6\u7801</label><input id="profModalNewPwd2" type="password" placeholder="\u518d\u6b21\u8f93\u5165"></div><button class="btn" onclick="submitProfilePwdModal()">\u4fee\u6539\u5bc6\u7801</button><div id="profModalPwdMsg" class="msg" style="display:none"></div></div></div><div id="changePasswordModal" class="modal" style="display:none"><div class="modal-content" style="max-width:380px"><span class="modal-close" onclick="document.getElementById('"'"'changePasswordModal'"'"').style.display='"'none'"'">&times;</span><h3>\u4fee\u6539\u7528\u6237\u5bc6\u7801</h3><p id="cpUserInfo" style="color:#666;margin-bottom:12px;font-size:13px"></p><div class="form-group"><label>\u65b0\u5bc6\u7801</label><input id="cpNewPwd" type="password" placeholder="\u81f3\u5c114\u4e2a\u5b57\u7b26"></div><button class="btn" onclick="submitAdminChangePwd()">\u786e\u8ba4\u4fee\u6539</button><div id="cpMsg" class="msg" style="display:none"></div></div></div>';document.body.appendChild(m.firstChild)}e.style.display="flex";fetch("/me",{credentials:"include"}).then(function(r){return r.json()}).then(function(u){document.getElementById("profModalUser").value=u.username||"";document.getElementById("profModalDN").value=u.display_name||"";document.getElementById("profModalRole").value=u.role||""})}
function closeProfile(){document.getElementById("profileModal").style.display="none"}
function saveProfileModal(){var dn=document.getElementById("profModalDN").value.trim(),msg=document.getElementById("profModalSaveMsg");msg.style.display="none";fetch("/me",{method:"POST",credentials:"include",headers:{"Content-Type":"application/json"},body:JSON.stringify({display_name:dn})}).then(function(r){return r.json()}).then(function(d){d.ok?(msg.textContent="\u5df2\u4fdd\u5b58",msg.style.color="#27ae60"):(msg.textContent=d.detail||"\u4fdd\u5b58\u5931\u8d25",msg.style.color="#e74c3c");msg.style.display="block";setTimeout(function(){msg.style.display="none"},3000)})}
function submitProfilePwdModal(){var o=document.getElementById("profModalOldPwd").value,n=document.getElementById("profModalNewPwd").value,n2=document.getElementById("profModalNewPwd2").value,m=document.getElementById("profModalPwdMsg");m.style.display="none";if(!o){m.textContent="\u8bf7\u8f93\u5165\u65e7\u5bc6\u7801";m.style.display="block";return}if(n.length<4){m.textContent="\u65b0\u5bc6\u7801\u81f3\u5c114\u4e2a\u5b57\u7b26";m.style.display="block";return}if(n!=n2){m.textContent="\u4e24\u6b21\u8f93\u5165\u4e0d\u4e00\u81f4";m.style.display="block";return}fetch("/me/change_password",{method:"POST",credentials:"include",headers:{"Content-Type":"application/json"},body:JSON.stringify({old_password:o,new_password:n})}).then(function(r){return r.json()}).then(function(d){d.ok?(m.textContent="\u5bc6\u7801\u5df2\u4fee\u6539",m.style.color="#27ae60",document.getElementById("profModalOldPwd").value="",document.getElementById("profModalNewPwd").value="",document.getElementById("profModalNewPwd2").value=""):(m.textContent=d.detail||"\u4fee\u6539\u5931\u8d25",m.style.color="#e74c3c");m.style.display="block";setTimeout(function(){m.style.display="none"},3000)})}
var _cu=0;
function adminChangePwd(u){_cu=u;document.getElementById("changePasswordModal").style.display="flex";document.getElementById("cpUserInfo").textContent="\\u7528\\u6237ID: "+u;document.getElementById("cpNewPwd").value="";document.getElementById("cpMsg").style.display="none"}
function submitAdminChangePwd(){var n=document.getElementById("cpNewPwd").value,m=document.getElementById("cpMsg");m.style.display="none";if(n.length<4){m.textContent="\u65b0\u5bc6\u7801\u81f3\u5c114\u4e2a\u5b57\u7b26";m.style.display="block";return}fetch("/admin/user/"+_cu+"/change_password",{method:"POST",credentials:"include",headers:{"Content-Type":"application/json"},body:JSON.stringify({new_password:n})}).then(function(r){return r.json()}).then(function(d){d.ok?(m.textContent="\u5bc6\u7801\u5df2\u4fee\u6539",m.style.color="#27ae60",setTimeout(function(){document.getElementById("changePasswordModal").style.display="none"},1500)):(m.textContent=d.detail||"\u4fee\u6539\u5931\u8d25",m.style.color="#e74c3c");m.style.display="block"})}
'''

# 在 _ADMIN 中添加侧边栏和表格密码列 + 头像可点击
new_html = new_html.replace(
    'data-page="page-users"',
    'data-page="page-profile" onclick="openProfile()"><span class="nav-icon">\U0001f464</span><span class="nav-text">\u4e2a\u4eba\u8d44\u6599</span></li><li class="nav-item" data-page="page-users"'
)
new_html = new_html.replace('</thead>', '<th style="width:100px">\u5bc6\u7801</th></thead>')
new_html = new_html.replace(
    'onclick="deleteUser(',
    '<button class="btn-small btn-pwd" onclick="adminChangePwd(${user.id})" style="margin-right:4px">\u4fee\u6539\u5bc6\u7801</button> <button onclick="deleteUser('
)
new_html = new_html.replace(
    '"><span class="user-area">\U0001f464; <!--U--></span>',
    '"><span class="user-area"><a onclick="openProfile()" style="cursor:pointer;color:inherit;text-decoration:none">\U0001f464; <!--U--></a></span>'
)
new_html = new_html.replace('</body>', '</body>')

c = c[:start] + new_html + c[end:]

# 在末尾添加 /api/app.js 路由
api_route = '\n\n@app.get("/api/app.js")\nasync def app_js():\n    return PlainTextResponse("""' + all_js.replace('"""', '\\"\\"\\"') + '""", media_type="application/javascript")\n'

name_pos = c.rfind('\nif __name__')
if name_pos < 0:
    name_pos = c.rfind('\nif __name__')
c = c[:name_pos] + api_route + c[name_pos:]

with open('railway_file_server.py', 'w', encoding='utf-8') as f:
    f.write(c)

import ast
ast.parse(c)
print(f'OK: {len(c)} bytes')
